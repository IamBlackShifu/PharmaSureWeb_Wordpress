...
final class ClaimsController {
	...
	public function can_insurers() { return $this->cap( 'pharmasure_manage_insurers' ); }
	public function can_prepare() { return $this->cap( 'pharmasure_prepare_claims' ); }
	public function can_submit() { return $this->cap( 'pharmasure_submit_claims' ); }
	public function can_reconcile() { return $this->cap( 'pharmasure_reconcile_claims' ); }
	...
	public function remittance( $r ) { $scope = $this->tenant(); return is_wp_error( $scope ) ? $scope : $this->respond( $this->service->reconcile( $scope, (array) $r->get_json_params(), get_current_user_id() ), 201 ); }
	...
	private function cap( $cap ) { return current_user_can( $cap ) ? true : new \WP_Error( 'forbidden', 'Claims permission is required.', array( 'status' => 403 ) ); }
