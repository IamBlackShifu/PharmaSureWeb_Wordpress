...
final class ClaimsService {
	...
	public function reconcile( $tenant_id, array $data, $actor_id ) {
		$insurer_id = absint( $data['insurer_id'] ?? 0 ); $reference = sanitize_text_field( $data['reference'] ?? '' ); $key = sanitize_text_field( $data['idempotency_key'] ?? '' ); $date = $this->date( $data['received_date'] ?? '' ); $lines = (array) ( $data['items'] ?? array() );
		...
		try {
			...
			foreach ( $lines as $line ) { $claim_id = absint( $line['claim_id'] ?? 0 ); $claim = $this->row( "SELECT * FROM {$this->p}claims WHERE id=%d AND tenant_id=%d AND insurer_id=%d FOR UPDATE", $claim_id, $tenant_id, $insurer_id ); $paid = max( 0, (int) ( $line['paid_amount_minor'] ?? 0 ) ); if ( ! $claim || ! in_array( $claim['status'], array( 'approved', 'partially_approved', 'partially_paid' ), true ) || $paid <= 0 || (int) $claim['paid_amount_minor'] + $paid > (int) $claim['approved_amount_minor'] ) { throw new \DomainException( 'Each remittance line must reference an approved claim and cannot overpay it.' ); } $total += $paid; $claims[] = array( $claim, $line, $paid ); }
