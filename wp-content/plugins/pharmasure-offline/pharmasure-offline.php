...
if(!defined('ABSPATH')){exit;}const VERSION='1.3.0';const DB_VERSION='3';
...
add_action('plugins_loaded',static function(){if(!class_exists('PharmaSure\Core\TenantContext')||!class_exists('PharmaSure\Integrations\Services\CredentialVault')){return;}if(!Installer::is_current_site_installed()){Installer::install();}Admin\OfflineAdmin::register_actions();add_action('rest_api_init',array(Rest\OfflineController::class,'register_routes'));add_action('admin_menu',array(Admin\OfflineAdmin::class,'register_page'));add_action('pharmasure_replay_offline_mutations',array(Services\ReplayService::class,'process_due'));},70);
add_action('wp_initialize_site',array(Installer::class,'install_for_site'),200);
