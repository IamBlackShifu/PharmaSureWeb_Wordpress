(() => {
  ...
  function accountsDashboard(){const data=state.accountsData,root=node('div',{className:'ps-accounts-root'});root.append(node('header',{className:'ps-accounts-title'},[node('div',{},[node('span',{className:'ps-eyebrow',text:'Identity / least privilege'}),node('h1',{text:'Accounts & access'}),node('p',{text:`${data.scope.pharmacy}. Staff identities, application roles and explicit branch boundaries.`})]),node('button',{className:'ps-operation-button ps-operation-button--primary',type:'button',text:'Create account',onClick:()=>openAccountDialog()})]));root.append(node('section',{className:'ps-accounts-metrics'},[reportMetric('Tenant accounts',data.metrics.total,'Visible only in this pharmacy'),reportMetric('Active',data.metrics.active,'Can authenticate','emerald'),reportMetric('Administrators',data.metrics.administrators,'All-branch authority'),reportMetric('Branch restricted',data.metrics.branch_restricted,'Explicit assignments')]));root.append(node('section',{className:'ps-accounts-grid'},[accountList(),accountInspector()]));return root;}
